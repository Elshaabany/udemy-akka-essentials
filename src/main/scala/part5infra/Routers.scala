  ...
  class Master extends Actor {
    ...
    private def onMessage(router: Router): Receive = {
      ...
      case Terminated(ref) =>
        ...
        context.become(onMessage(newRouter.addRoutee(newSlave)))
...
//  val master = system.actorOf(Props[Master], "theMaster")
//
//  for (i <- 1 to 10) {
...
//    }
  ...
  // actors must be created, only the path is send form the conf
...
//   * Special messages
//   */
  groupMaster2 ! Broadcast(Kill)

  for (i <- 1 to 10) {
    groupMaster2 ! s"[$i] still alive!"
  }
