...
class InterceptingLogsSpec extends TestKit(
  ...
  // to adjust the timeout of eventFilter we set akka.test.filter-leeway to higher value in conf
  // we should configure the logger to make EventFilter able to query it

  // by this way we achieved integration tests based on log messages

  "A checkout flow" should {
    "correctly log the dispatch of an order" in {
      // event filter is used to test pattern of logs
