...
class SupervisionSpec extends TestKit(ActorSystem("SuperVisionSpec", ConfigFactory.load().getConfig("interceptingLogMessages")))
  with ImplicitSender
  with WordSpecLike
  ...
  "A supervisor" should {
    "resume its child in case of a minor fault" in {
      ...
      // resume wil: ignore failure, continue processing message, keep the state
      child ! Report
      expectMsg(3)
    }
    ...
    "restart its child in case of an empty sentence" in {
      ...
      child ! ""
      // restart will start new actor with new state
      child ! Report
      expectMsg(0)
    }
    ...
    "terminate its child in case of a major error" in {
      ...
      watch(child)
      child ! "akka is nice"
      // stop directive will: stop the failed actor permanently
    ...
    "escalate an error when it doesn't know what to do" in {
      ...
      watch(child)
      child ! 43
      // the default behaviour of escalate will terminate the failing child actor
  ...
  "A kinder supervisor" should {
    "not kill children in case it's restarted or escalates failures" in {
      val supervisor = system.actorOf(Props[NoDeathOnRestartSupervisor], "newSupervisor")
      supervisor ! Props[FussyWordCounter]
      val child = expectMsgType[ActorRef]
      ...
      child ! 45
      child ! Report
      // will not stop the child actor permanently
      expectMsg(0)
    }
  }
  ...
  "An all-for-one supervisor" should {
    "apply the all-for-one strategy" in {
      ...
      // will restart all the child actors not only the causing failure
